...
export default function CreatePage() {
  ...
  async function onChangeSubmit(event) {
    ...
    let infocreate = await axios.post(
      `${process.env.REACT_APP_BACKEND_URL}/api/pokemons/pokemonscreate`,
      input
    );
  ...
  return (
    <div className="bgcreate">
      ...
      <h1 className="titulo">Show us your Pokémon!</h1>
        ...
        <form onSubmit={(e) => onChangeSubmit(e)}>
          <div className="inputcreacion">
              ...
              placeholder="Name"
                ...
                required
              ></input>
            <label>{error.name && <p>{error.name}</p>}</label>
          </div>
          <div className="inputcreacion">
              <input
              placeholder="Image"
          ...
          <div className="divtypes">
            ...
            {input.types?.map((t, index) => (
              <div className="opcionestype" key={index}>
                <button type="button" onClick={() => onHandleClose(t)}>
                  x
                </button>
